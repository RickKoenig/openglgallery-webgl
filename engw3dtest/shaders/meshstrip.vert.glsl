...
// no attributes, generated by keyword gl_VertexID
...
void main() {
	int VPR = 2 * (numCols + 1);
	int yAlt = gl_VertexID % 2;
	int yRow = gl_VertexID / VPR;
	int row = yAlt + yRow;
	int col = gl_VertexID / 2 % (numCols + 1);
	vPos = vec2(col, row) / vec2(numCols, numRows); // 0 to 1
